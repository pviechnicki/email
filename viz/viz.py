...
import sys
sys.path.insert(0, 'C:/Users/ComputerA/email_marker/src/model')
import os
...
#read in csv
raw_df = pd.read_excel('C:/Users/ComputerA/email_marker/src/model/email_df.xlsx', sheetname = 'Sheet1')
...
initialize_hour_tallies(hour_tallies, my_cat_labels, my_orgs, range(1,60))
#Now fill up with actual numbers
for index, row in raw_df.iterrows():
    row['sent_date'] = dt.datetime.strptime(row['sent_date'], "%Y-%m-%dT%H:%M:%S")
    hour_tallies[row['sensitivity']][row['org_unit']][row['sent_date'].minute] += 1
    hour_tallies[row['sensitivity']]['All'][row['sent_date'].minute] += 1
    hour_tallies['All'][row['org_unit']][row['sent_date'].minute] += 1
    hour_tallies['All']['All'][row['sent_date'].minute] += 1
                    ...
                    'data': [
                        go.Bar(
                            ...
                            y = list(map(safe_divide,
                                         hour_tallies['Sensitivity Personal'].values(),
                        ...
                        go.Bar(
                            x=list(range(1,60)),
                            y = list(map(safe_divide,
                                         hour_tallies['Sensitivity Official'].values(),
                                         hour_tallies['All']['All'].values())),
                            name='Official',
                            marker={'color': '#1f77b4'}
                        )
